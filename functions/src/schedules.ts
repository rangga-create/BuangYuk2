...
async function getUsersInRegion(city: string, district: string): Promise<string[]> {
  const tokens: string[] = [];
...
export const scheduleReminderH0 = functions.pubsub.schedule('0 7 * * *').timeZone('Asia/Jakarta').onRun(async (context) => {
...
export const scheduleReminderH1 = functions.pubsub.schedule('0 19 * * *').timeZone('Asia/Jakarta').onRun(async (context) => {
