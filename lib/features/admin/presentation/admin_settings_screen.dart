...
class _AdminSettingsScreenState extends State<AdminSettingsScreen> {
  ...
  bool _autoAssignCollectors = false;
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildProfileSection(isDark, admin),
                    SizedBox(height: 24.h),
                    _buildPreferencesSection(isDark),
                    SizedBox(height: 24.h),
                    _buildSystemSection(isDark),
                    SizedBox(height: 24.h),
  ...
  Widget _buildProfileSection(bool isDark, Map<String, dynamic> admin) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        GlassCard(
          child: Column(
            children: [
              _buildInfoRow(isDark, 'Nama', admin['name']),
              const Divider(height: 1),
              _buildInfoRow(isDark, 'Email', admin['email']),
              const Divider(height: 1),
              _buildInfoRow(isDark, 'Jabatan', admin['position']),
              const Divider(height: 1),
              _buildInfoRow(isDark, 'Wilayah', admin['region']),
            ],
          ),
    ...
    return Column(
      ...
      children: [
        ...
        GlassCard(
          child: Column(
            children: [
              _buildToggleTile(isDark, 'Notifikasi Push', 'Terima notifikasi sistem', _notificationsEnabled, (v) {
                setState(() => _notificationsEnabled = v);
              }),
              const Divider(height: 1),
              _buildToggleTile(isDark, 'Laporan Email', 'Terima laporan via email', _emailReports, (v) {
                setState(() => _emailReports = v);
              }),
              const Divider(height: 1),
              _buildToggleTile(isDark, 'Auto-Assign Kolektor', 'Tugaskan petugas otomatis', _autoAssignCollectors, (v) {
                setState(() => _autoAssignCollectors = v);
              }),
            ],
          ),
    ...
    return Column(
      ...
      children: [
        ...
        GlassCard(
          child: Column(
            children: [
              _buildToggleTile(isDark, 'Mode Pemeliharaan', 'Nonaktifkan akses pengguna sementara', _maintenanceMode, (v) {
                setState(() => _maintenanceMode = v);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(v ? 'Mode pemeliharaan diaktifkan' : 'Mode pemeliharaan dinonaktifkan'),
                    behavior: SnackBarBehavior.floating,
                  ),
                );
              }),
              const Divider(height: 1),
              _buildActionTile(isDark, Icons.download_rounded, 'Ekspor Data Laporan', 'PDF, Excel, CSV'),
              const Divider(height: 1),
              _buildActionTile(isDark, Icons.restart_alt_rounded, 'Reset Statistik', 'Hapus data sementara'),
            ],
          ),
    ...
    return Column(
      ...
      children: [
        ...
        GlassCard(
          child: Column(
            children: [
              _buildInfoRow(isDark, 'Versi Aplikasi', '1.0.0'),
              const Divider(height: 1),
              _buildInfoRow(isDark, 'Database', 'Terhubung'),
            ],
          ),
